...
class ContainerProduct extends Component {
    ...
    render() { 
        return (
            <Container className="container-produk d-flex flex-wrap ps-4 pt-3">
                {this.props.dataImg.map(data => 
                    <Card ref={this.Card} key={data.id} id="baju" style={{ width: '13rem'}} className="ms-4 p-1 mt-4 d-block h-50 shadow-sm card" onClick={() => this.props.onClickId(data.id)}>
